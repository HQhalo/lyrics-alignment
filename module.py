...
class AlignmentEncoderExtend(torch.nn.Module):
    """Module for alignment text and mel spectrogram. """

    def __init__(
        self, jasper_encoder,  decoder_feat_in = 1024, n_text_channels=512, n_att_channels=128, temperature=0.0005,
    ):
        super().__init__()
        ...
        self.key_proj = nn.Sequential(
            ...
            torch.nn.ReLU(),
            ConvNorm(n_text_channels * 2, n_text_channels * 2, kernel_size=3, bias=True),
            torch.nn.ReLU(),
        ...
        self.decoder_layers = nn.Sequential(
            ConvNorm(decoder_feat_in, n_att_channels, kernel_size=1, bias=True),
        )

    def get_dist(self, keys, queries, queries_len, mask=None):
        ...
        queries_enc = self.decoder_layers(self.jasper_encoder(queries, queries_len)[0])   # B x n_attn_dims x T1
    ...
    def forward(self, queries, queries_len, keys, mask=None, attn_prior=None, conditioning=None):
        ...
        keys_enc = self.key_proj(keys)  # B x n_attn_dims x T2
        kwarg = {"audio_signal": queries, "length": queries_len}
        otmp = self.jasper_encoder(**kwarg)
        queries_enc = self.decoder_layers(otmp[0])  # B x n_attn_dims x T1
